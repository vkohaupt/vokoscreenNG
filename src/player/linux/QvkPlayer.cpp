...
QvkPlayer::QvkPlayer( QMainWindow *parent, Ui_formMainWindow *ui_formMainWindow ) : ui_player(new Ui_player)
{
...
/*
    if ( isFullScreen() == false )
    {
        ...
        {
            qDebug() << "11111111111111111111111111111111111111111111111111111111111111111111111";

            ui_gui->verticalLayout_gui_player->addWidget( this );

            widget_Video->setUpdatesEnabled( true );
            ui_player->widget_menuebar->setParent( widget_Video );
            ui_player->widget_menuebar->raise();
            ui_player->widget_menuebar->show();


            ui_player->verticalLayout_player->addWidget( widget_Video, 1 );
            ui_player->verticalLayout_player->addWidget( ui_player->widget_menuebar, 0 );
            showFullScreen();
            return;
        }
        ...
        {
            widget_Video->setUpdatesEnabled( true );
            ui_player->widget_menuebar->setParent( ui->label_logo );
            ui_player->widget_menuebar->raise();
            ui_player->widget_menuebar->show();
            showFullScreen();
        }
...
/*
QvkPlayer::QvkPlayer( QMainWindow *parent, Ui_formMainWindow *m_ui ) : ui(new Ui::player)
{
    Q_UNUSED(parent)
...
void QvkPlayer::slot_embedded( bool embedded )
{
    if ( embedded == true )
    {
        ui->verticalLayoutTabSidebarPlayer->insertWidget( 0, widget_Video, 1 );
        ui->verticalLayoutTabSidebarPlayer->insertWidget( 1, ui->label_logo, 1 );
        ui->verticalLayoutTabSidebarPlayer->insertWidget( 2, ui->widget_menuebar, 0 );
        widget_Video->setUpdatesEnabled( false );
        hide();
    } else {
...
void QvkPlayer::slot_stop()
{
    ...
    if ( ui->pushButtonEmbedded->isChecked() == false )
    {
        ...
        ui->verticalLayoutTabSidebarPlayer->insertWidget( 0, widget_Video );
        ui->verticalLayoutTabSidebarPlayer->setStretch( 0, 1 );;
    }
...
z    if ( ui->pushButtonEmbedded->isChecked() == true )
    {
        slot_embedded( true );
    }
}
...
void QvkPlayer::keyPressEvent( QKeyEvent *event )
{
    ...
    if ( ( ( event->key() == Qt::Key_F11 ) or ( event->key() == Qt::Key_F ) or ( event->key() == Qt::Key_Escape ) ) and ( isFromGui == true ) ) {
        QMouseEvent* doubleClick = new QMouseEvent( QEvent::MouseButtonDblClick, ui->tabSidebarPlayer->rect().center(), Qt::LeftButton, Qt::LeftButton, Qt::NoModifier );
        QApplication::postEvent( ui->tabSidebarPlayer, doubleClick );
        isFromGui = false;
        return;
    }
