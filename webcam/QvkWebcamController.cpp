...
QvkWebcamController::QvkWebcamController( Ui_MainWindow *value )
{
    ...
    connect( vkWebcamWatcher, SIGNAL( signal_new_list_a_camera_was_added( QList<QCameraInfo> ) ), this, SLOT( slot_new_camera_list( QList<QCameraInfo> ) ) );
    connect( ui_vokoscreen->ComboBoxCamera, SIGNAL( currentIndexChanged( int ) ), this, SLOT( slot_setCamera() ) );
...
void QvkWebcamController::slot_setCamera()
{
    delete camera;
    delete webcamWindow;
